...
shell-connect-arm-discovery-ambiguous = Multiple ARM Cosmos DB accounts match the connected endpoint. Pass --connect-subscription and --connect-resource-group (or the equivalent options on the 'connect' command) to use Azure Resource Manager; using Cosmos DB data plane.
