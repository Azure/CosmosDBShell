...
help-OutputFormat = Output format for machine-readable output: json (raw array), ndjson (one document per line), json-full (structured envelope), or table (default).
help-Quiet = Suppress non-result status messages. Implies machine mode; use with --output json/ndjson/json-full for clean pipeline output.
