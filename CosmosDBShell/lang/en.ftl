...
shell-connect-arm-discovery-failed = Could not discover the Azure Resource Manager context for this account ({ $message }). Data-plane operations will continue to work; resource commands (mkdb, mkcon, rmdb, rmcon, indexpolicy) will require reconnecting with --subscription, --resource-group, and --account.
...
command-connect-description-account = Cosmos DB account name for ARM database and container operations.
