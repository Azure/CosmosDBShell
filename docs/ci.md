...
Classification is centralized in `ShellExitCode`.

| Code | Meaning | Typical causes |
| ---- | ------- | -------------- |
| `0` | Success | The command or script completed without error. |
| `1` | General failure | An error that does not fit a more specific category. |
| `2` | Usage error | CLI parse failure, script syntax error, unknown/bad arguments, or an unrecognized option. |
| `3` | Authentication failure | Invalid, missing, or expired credentials; `401 Unauthorized`; `403 Forbidden`; a failed Entra credential. |
| `4` | Connection/network error | DNS/socket failure, `503 Service Unavailable`, or a request timeout (`408`/`504`). |
| `5` | Not found | Cosmos / ARM `404 Not Found`. |
| `6` | Throttled | Cosmos `429 Too Many Requests` / RU budget exceeded. |
...
case $? in
  0) echo "ok" ;;
  2) echo "fix the command line or script"; exit 1 ;;
  3) echo "check credentials"; exit 1 ;;
  4) echo "transient connectivity issue"; exit 1 ;;
  5) echo "resource not found"; exit 1 ;;
  6) echo "throttled / retry with backoff"; exit 1 ;;
...
Exit code `4` (connection) and sometimes `6` (throttle) or `1` indicate a
...
for attempt in 1 2 3; do
  ...
  code=$?
  # Retry connectivity/throttle; fail fast on usage or auth errors.
  if [ "$code" -ne 4 ] && [ "$code" -ne 6 ]; then exit "$code"; fi
  sleep $((attempt * 5))
done
```
...
Request JSON output and pipe it to a JSON processor. Prefer the global
`--output json` / `-o json` option (or per-command `--format json`). Bare `-c`
also defaults to JSON in machine mode:

```bash
count=$(cosmosdbshell --output json -c "query \"SELECT VALUE COUNT(1) FROM c\"" | jq '.[0]')
