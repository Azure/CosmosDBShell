...
- The `COSMOSDB_SHELL_FORMAT` environment variable has been removed. The output format now comes solely from the global `--output` option or a per-command `--format` option. ([#173](https://github.com/Azure/CosmosDBShell/pull/173))
- The structured JSON emitted by `ls` and `connect` changed shape. `ls` now returns an object — `{ "type": "database" | "container" | "item", "values": [...] }` — instead of a bare array for databases and containers and `{ "items": [...] }` for items, and item listings carry an additional `limitReached` flag. A successful `connect <endpoint>` now returns `{ "connected": true, "endpoint": "..." }` instead of `{ "connected state": "..." }`. Tooling that parses this output must be updated. ([#173](https://github.com/Azure/CosmosDBShell/pull/173))
