...
- `throughput` command to view and scale provisioned RU/s on the current database or container (or a specific one via `--database`/`--container`): `show` (current mode, provisioned RU/s, autoscale maximum, and minimum as JSON), `set`/`manual` (switch to manual provisioning at a given RU/s), and `autoscale` (switch to autoscale with a given maximum RU/s), with a confirmation prompt that `--yes` bypasses. ([#109](https://github.com/Azure/CosmosDBShell/issues/109))
...
- `import` and `export` commands for bulk data roundtrip. `import` bulk-loads items from a local JSON Lines, JSON array, or CSV file (`--format`, `--mode=insert|upsert`, `--partition-key` to nest a CSV column under a nested partition key path, `--continue-on-error`, and `--dry-run` to validate without writing); `export` streams container items to a JSON Lines, JSON array, or CSV file, optionally filtered by `--query` and capped with `--max`, with `--force` to overwrite an existing file.
- `index` command to manage a container's indexing policy: `show` (current policy as JSON), `add <path...>` and `remove <path...>` (adjust the included/excluded paths using Cosmos indexing path syntax such as `/address/*`), and `set` (patch `--mode` and/or `--automatic`, or replace the whole policy with a JSON document). ([#101](https://github.com/Azure/CosmosDBShell/issues/101))
- `watch` command (alias `tail`) to follow a container's change feed live: `--from-beginning` replays from the start of the container, `--partition-key` scopes to a single partition key, `--max` stops after N changes, `--interval` sets the poll interval (default 1s), and `--database`/`--container` target a specific resource. ([#100](https://github.com/Azure/CosmosDBShell/issues/100))
- `edit` command to open a local file (for example a `.csh` script) in the external editor and wait for it to close, creating the file when it does not yet exist. Available only in an interactive session. ([#110](https://github.com/Azure/CosmosDBShell/issues/110))
